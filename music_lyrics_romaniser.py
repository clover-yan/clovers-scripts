...
def dump_additional_lines(additional_lines, timestamp):
	...
	if additional_lines:
		if args.style == 'salt':
			result += '\n'.join((timestamp + i) for i in additional_lines) + '\n'
		else:
...
def romanise_lyrics(text):
	...
	for line in text.split('\n'):
		...
		if line_content.strip() and not args.convert_only:
			line_romanised = []
			for item in kks.convert(line_content):
				line_romanised.append(item['hepburn'])
			additional_lines.append(' '.join(line_romanised))
		result += line + '\n'
	result += dump_additional_lines(additional_lines, line_timestamp)
	return result
